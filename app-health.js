...
function applyLayoutToGrid() {
  const grid = elements.healthGrid;
  if (!grid) return;
...
function renderHealth() {
  const healthTools = sortTools(
    state.tools.filter((tool) => tool.category === "Health")
  );
  elements.healthGrid.innerHTML = "";

  if (healthTools.length === 0) {
    elements.healthGrid.appendChild(
      createInlineEmptyState("Add tools with category Health (e.g. Apple Fitness, Pedometer++, Nourish, Fitbod) to see them here.")
    );
    return;
  }
