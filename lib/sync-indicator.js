...
import { initFirebase, getAuthInstance } from "./firebase.js";
...
let bootstrappedThisSession = false;

async function bootstrapSessionSync() {
  if (bootstrappedThisSession) return;
  bootstrappedThisSession = true;
  try {
    await initFirebase();
    const auth = getAuthInstance();
    // Firebase persists the signed-in user across reloads; if a user is
    // already present after init, the app is about to hit Firestore.
    if (auth?.currentUser) {
      markSyncStart("Connecting to cloud…");
    }
  } catch {
    // no firebase configured; stay idle
  }
}
