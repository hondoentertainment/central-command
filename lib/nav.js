...
export function renderNav(current, options = {}) {
  ...
  const integrationEntries = listIntegrations(integrationPrefs);
  const navIntegrations = integrationEntries.filter(
    (entry) => isIntegrationConfigured(entry.config) && entry.config.showInNav
  );
  const integrationById = new Map(integrationEntries.map((e) => [e.id, e]));
  ...
  for (const entry of navIntegrations) {
    browseGroups[0].links.push({
      key: `integration-${entry.id}`,
      href: entry.config.url,
  ...
  container.innerHTML = `
    <div class="topbar">
        ...
        <div class="topbar__nav-browse">
          ${browseGroups.map((group) => `
            <div class="topbar__browse-group">
              ...
              ${group.links.map((link) => renderNavLink(link, current, integrationById)).join("")}
            </div>
          `).join("")}
        </div>
      </div>
      <div class="topbar__right">
        ...
        <div class="page-nav__sync-slot"></div>
  ...
  const syncSlot = container.querySelector(".page-nav__sync-slot");
  if (syncSlot) renderSyncIndicator(syncSlot);

  container.querySelectorAll("[data-integration-id]").forEach((link) => {
    link.addEventListener("click", (event) => {
      const entry = integrationById.get(link.getAttribute("data-integration-id"));
      if (!entry) return;
      event.preventDefault();
      openIntegration(entry.config, entry.definition, {
        source: current,
...
function renderNavLink(link, current, integrationById) {
  const active = link.key === current ? " is-active" : "";
  const ariaCurrent = link.key === current ? ' aria-current="page"' : "";
  const entry = link.integrationId ? integrationById?.get(link.integrationId) : null;
  const openMode = entry?.config?.openMode;
  const target = link.external && openMode !== "same-tab" ? ' target="_blank" rel="noreferrer"' : "";
  const dataIntegration = link.integrationId ? ` data-integration-id="${link.integrationId}"` : "";
  return `<a class="topbar__link${active}" href="${link.href}"${target}${ariaCurrent} data-key="${link.key}"${dataIntegration}>${link.label}</a>`;
}
