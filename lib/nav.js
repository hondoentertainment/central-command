...
export function renderNav(current, options = {}) {
  ...
  const workspaceLinks = browseGroups.flatMap((g) => g.links);
    ...
    container.innerHTML = `
      ...
      <div class="sidebar-section sidebar-section--secondary">
        ...
        ${browseGroups.map((group) => {
          const hasActive = group.links.some((l) => l.key === current);
          return `
          <details class="sidebar-browse-group" ${hasActive ? "open" : ""}>
            <summary class="sidebar-browse-group__toggle">${group.label}</summary>
            <div class="sidebar-browse-group__links">
              ${group.links.map((link) => renderLink(link, current, creativeHub)).join("")}
            </div>
          </details>`;
        }).join("")}
      </div>
