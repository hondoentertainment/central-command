...
/**
 * Safely write to localStorage. Shows a toast on QuotaExceededError.
 * @param {string} key
 * @param {string} value
...
/**
 * Load raw tool data from current or legacy localStorage keys.
 * @returns {any}
 */
...
/**
 * Snapshot the current Firebase user for sync policy checks.
 * @returns {{ uid: string, isAnonymous: boolean, emailVerified: boolean }|null}
 */
...
/**
 * Ensure every tool in the array has an updatedAt timestamp.
 * @param {Array} tools
 * @returns {Array}
 */
