...
export function initCategoryHubPage(config) {
  const { navKey, gridSelector, category, emptyStateMessage } = config;
  ...
  renderCategoryGrid(grid, state, category, emptyStateMessage);
}
 ...
 */
function renderCategoryGrid(grid, state, category, emptyStateMessage) {
  const filtered = sortTools(state.tools.filter((tool) => tool.category === category));
  grid.innerHTML = "";

  if (filtered.length === 0) {
    grid.appendChild(createInlineEmptyState(emptyStateMessage));
    return;
  }
