...
/**
 * Reads the stored integration event log (safe, returns [] on any error).
 */
export function readIntegrationEvents() {
  try {
    const raw = JSON.parse(localStorage.getItem(INTEGRATION_EVENT_KEY) || "[]");
...
/**
 * Aggregates integration events into per-integration stats. Events without an
 * integrationId payload fall back to reverse-mapping the eventName to a
 * definition (useful for pre-generalization creative_hub_link_opened rows).
 * Returns an array in the same order as INTEGRATION_DEFINITIONS, even for
 * integrations with zero events, so UI tables stay stable.
 *
 * @param {Array} [events] - optional event list; defaults to readIntegrationEvents()
 * @returns {Array<{id: string, name: string, icon: string, opens: number, lastOpenedAt: string|null, lastSource: string|null}>}
 */
export function aggregateIntegrationEvents(events = readIntegrationEvents()) {
  const byId = new Map();
  for (const def of INTEGRATION_DEFINITIONS) {
    byId.set(def.id, {
      id: def.id,
  ...
  const eventNameToId = new Map(INTEGRATION_DEFINITIONS.map((d) => [d.eventName, d.id]));

  for (const entry of events) {
    if (!entry || typeof entry !== "object") continue;
    const payload = entry.payload && typeof entry.payload === "object" ? entry.payload : {};
    const id = payload.integrationId || eventNameToId.get(entry.eventName);
    if (!id || !byId.has(id)) continue;
    const row = byId.get(id);
    row.opens += 1;
    // Events are appended in insertion order; ties on timestamp (same ms) are
    // broken by keeping the more recent event so the "last source" reflects
    // true event ordering rather than lexical timestamp comparison.
    const ts = entry.createdAt;
    if (ts && (!row.lastOpenedAt || ts >= row.lastOpenedAt)) {
      row.lastOpenedAt = ts;
      row.lastSource = typeof payload.source === "string" ? payload.source : null;
    }
  }

  return INTEGRATION_DEFINITIONS.map((def) => byId.get(def.id));
}

export function clearIntegrationEvents() {
  try {
    localStorage.removeItem(INTEGRATION_EVENT_KEY);
  } catch {
    // no-op
  }
}
