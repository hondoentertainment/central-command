...
/**
 * Load Firebase config. Tries firebase.config.local.js first (root or config/), falls back to example.
 * @returns {Promise<Object|null>}
 */
async function loadFirebaseConfig() {
  for (const path of ["../firebase.config.local.js", "../config/firebase.config.local.js"]) {
    try {
      const local = await import(path).then((m) => m.default);
