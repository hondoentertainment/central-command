  ...
  overlay.innerHTML = `
    <div class="command-palette-panel">
      <div class="command-palette-search">
        ...
        <input type="search" class="command-palette-input" placeholder="Search tools, categories, runbook…" autocomplete="off" aria-label="Search tools and runbook" />
      </div>
...
export function initCommandPalette() {
  ...
  function updateSelectionDisplay() {
    ...
    if (resultsEl) {
      if (selectedIndex >= 0 && items[selectedIndex]) {
        ...
        resultsEl.setAttribute("aria-activedescendant", optionId);
      } else {
  ...
  function activateItem(item) {
    if (item.type === "tool") {
      ...
      openIntegration(item.integration, item.definition, {
  ...
  function renderResults() {
    ...
    const integrationEntries = listIntegrations(loadIntegrationsPreferences());
    const queryText = query.trim().toLowerCase();
    const toolNameSet = new Set(tools.map((tool) => tool.name.toLowerCase()));
    const paletteIntegrations = integrationEntries.filter((entry) => {
      if (!isIntegrationConfigured(entry.config) || !entry.config.showInCommandPalette) return false;
      if (toolNameSet.has(entry.definition.name.toLowerCase())) return false;
      if (!queryText) return true;
      const blob = `${entry.definition.name} ${entry.definition.description} integration open`.toLowerCase();
      return blob.includes(queryText);
    });

    // Order: integrations → pages → tasks → tools → subtasks → runbook
    items = [];
    for (const entry of paletteIntegrations) {
      items.push({
        type: "integration",
        title: `Open ${entry.definition.name}`,
        integration: entry.config,
