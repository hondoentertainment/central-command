...
function applyLayoutToGrid() {
  const grid = elements.agentsGrid;
  if (!grid) return;
...
function renderAgents() {
  const agentsTools = sortTools(
    state.tools.filter((tool) => tool.category === "Agents")
  );
  elements.agentsGrid.innerHTML = "";

  if (agentsTools.length === 0) {
    elements.agentsGrid.appendChild(
      createInlineEmptyState("Add tools with category Agents (e.g. Dreamer, Manus, GetViktor, OpenClaw) to see them here.")
    );
    return;
  }

  agentsTools.forEach((tool) => {
