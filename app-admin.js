...
function renderAdmin() {
  const adminTools = sortTools(
    state.tools.filter((tool) => tool.category === "Admin")
  );
  elements.adminGrid.innerHTML = "";

  if (adminTools.length === 0) {
    elements.adminGrid.appendChild(
      createInlineEmptyState("Add tools with category Admin (e.g. GitHub, Vercel, CI dashboards) to see them here.")
    );
    return;
  }
  ...
  adminTools.forEach((tool) => {
    ...
    elements.adminGrid.appendChild(card);
  });
}
